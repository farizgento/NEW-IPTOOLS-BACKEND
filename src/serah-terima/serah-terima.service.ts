...
import { JUMLAH_GRUP } from '../penilaian/penilaian';
import {
  alamatTautan,
  alatBelumDinilai,
  BATAS_PERCOBAAN_KODE,
  bacaTautan,
  buatKodeKonfirmasi,
  buatTautan,
  kodeCocok,
...
export interface PermintaanSiapkan {
  peminjamanId: number;
  arah: 'KIRIM' | 'KEMBALI';
  ringkas?: boolean;
...
@Injectable()
export class SerahTerimaService {
  private readonly log = new Logger(SerahTerimaService.name);

  constructor(
    private readonly repo: SerahTerimaRepository,
    private readonly pengguna: PenggunaRepository,
    private readonly notifikasi: NotifikasiRepository,
  ...
  async antreanPetugas(pengguna: Pengguna) {
    if (!this.adalahPengelola(pengguna)) {
      throw new ForbiddenException('Hanya pengelola alat yang dapat melihat daftar serah terima');
    }
  ...
  async persiapan(peminjamanId: number, pengguna: Pengguna) {
    if (!this.adalahPengelola(pengguna)) {
      throw new ForbiddenException('Hanya pengelola alat yang dapat menyiapkan serah terima');
    }
    if (pengguna.unitId === null) throw new ForbiddenException('Unit Anda belum tercatat');
    const data = await this.repo.persiapan(peminjamanId, pengguna.unitId);
    if (!data) throw new NotFoundException(`Pengajuan #${peminjamanId} tidak ditemukan`);
    ...
    const dinilai = await this.repo.alatDinilai(data.alat.map((x) => x.peminjamanAlatId), 'SENT', JUMLAH_GRUP);
  ...
  private adalahPengelola(pengguna: Pengguna): boolean {
    return pengguna.peran.includes('PENGELOLA') || pengguna.peran.includes('STAF');
  }

  /** F7 — menyiapkan dokumen serah terima. */
  async siapkan(pengguna: Pengguna, permintaan: PermintaanSiapkan) {
    if (!this.adalahPengelola(pengguna)) {
      throw new ForbiddenException('Hanya pengelola alat yang dapat menyiapkan serah terima');
    }
    ...
    const { tahap } = syaratPenilaian(permintaan.arah, 0);
    const dinilai = await this.repo.alatDinilai(permintaan.peminjamanAlatIds, tahap, JUMLAH_GRUP);
    const belum = alatBelumDinilai(permintaan.peminjamanAlatIds, dinilai);
    if (belum.length) {
      const { code, message } = syaratPenilaian(permintaan.arah, belum.length);
      throw new BadRequestException({ code, message, details: { peminjamanAlatIds: belum } });
    }

    const bentuk = permintaan.ringkas ? 'RINGKAS' : 'PENUH';
    const kode = buatKodeKonfirmasi();
  ...
  /** F7 — menyerahkan alat. */
  async serahkan(pengguna: Pengguna, serahTerimaId: number) {
    if (!this.adalahPengelola(pengguna)) {
      throw new ForbiddenException('Hanya pengelola alat yang dapat menyerahkan alat');
    }
    const st = await this.repo.ambil(serahTerimaId);
    if (!st) throw new NotFoundException(`Serah terima #${serahTerimaId} tidak ditemukan`);
    if (st.status !== 'DRAFT') {
      throw new BadRequestException({
    ...
    const status = await this.repo.serahkan({
      ...
      oleh: pengguna.id,
      nama: pengguna.nama,
      hitungStatus: (jumlah, sudah) => statusSetelahSerah(jumlah, sudah, st.arah),
    });
    ...
    const tautan = st.peminjamId && st.arah === 'KIRIM'
      ? alamatTautan(this.konf.WEB_URL, buatTautan({ serahTerimaId, penerimaId: st.peminjamId }, this.konf.JWT_RAHASIA))
      : null;
    if (tautan) await this.kirimTautan(st.peminjamId!, st.peminjamanId, serahTerimaId, st.jumlahAlat, tautan);
  ...
  private async kirimTautan(penerimaId: number, peminjamanId: number, serahTerimaId: number, jumlahAlat: number, tautan: string) {
    try {
      ...
      await this.notifikasi.antrekan({
        ...
        isi:
          `${penerima?.nama ?? 'Peminjam'}, ${jumlahAlat} alat pada peminjaman #${peminjamanId} sudah diserahkan. ` +
      ...
      this.log.warn(`Notifikasi tautan untuk serah terima #${serahTerimaId} gagal diantrekan: ${(e as Error).message}`);
    }
  }

  /** Halaman tautan tanpa login (PRD F8 jalur 1): hanya serah terima yang tertulis di tautan. */
  async berkasTautan(token: string) {
    const isi = this.bacaTautanSah(token);
    const st = await this.repo.ambil(isi.serahTerimaId);
    if (!st || st.peminjamId !== isi.penerimaId) throw new NotFoundException('Serah terima tidak ditemukan');
  ...
  async konfirmasiTautan(token: string, masukan: Pick<PermintaanKonfirmasi, 'serahTerimaAlatIds' | 'belumDiterima'>) {
    const isi = this.bacaTautanSah(token);
    const penerima = await this.pengguna.cariLewatId(isi.penerimaId);
    if (!penerima) throw new ForbiddenException('Penerima tautan tidak dikenali');
    return this.konfirmasi(isi.serahTerimaId, penerima, { metode: 'TAUTAN', tautan: token, ...masukan });
  }

  private bacaTautanSah(token: string) {
    const hasil = bacaTautan(token, this.konf.JWT_RAHASIA);
    if (!hasil.sah || !hasil.isi) throw new ForbiddenException(hasil.alasan ?? 'Tautan tidak sah');
    return hasil.isi;
  }

  /** F9 — daftar serah terima yang menunggu konfirmasi pengguna ini. */
  async menungguKonfirmasi(pengguna: Pengguna) {
  ...
  async berkas(serahTerimaId: number, pengguna: Pengguna) {
    const st = await this.repo.ambil(serahTerimaId);
    if (!st) throw new NotFoundException(`Serah terima #${serahTerimaId} tidak ditemukan`);

    const miliknya = st.peminjamId === pengguna.id;
    if (!miliknya && !this.adalahPengelola(pengguna)) {
      throw new ForbiddenException('Serah terima ini bukan milik Anda');
    }
  ...
  async konfirmasi(
    ...
    const izin = periksaMetode(permintaan.metode, pengelola, permintaan.alasan);
    ...
    const hasil = await this.repo.konfirmasi({
      ...
      hitungStatus: (jumlah, sudah) => statusSetelahTerima(jumlah, sudah, st.arah),
      belumDiterima: (permintaan.belumDiterima ?? []).filter((b) => belum.includes(b.serahTerimaAlatId) && !dipilih.includes(b.serahTerimaAlatId)),
    });

    this.log.log(
      `Serah terima #${serahTerimaId}: ${hasil.dikonfirmasi} alat dikonfirmasi ` +
  ...
  private async periksaKode(serahTerimaId: number, kode: string | null | undefined) {
    if (!kode?.trim()) {
      throw new BadRequestException({ code: 'KODE_KOSONG', message: 'Kode konfirmasi wajib diisi' });
    }
    const tersimpan = await this.repo.kodeKonfirmasi(serahTerimaId);
    if (tersimpan.percobaan >= BATAS_PERCOBAAN_KODE) {
      throw new ForbiddenException(
        'Percobaan kode sudah terlalu banyak. Hubungi pengelola alat untuk konfirmasi.',
      );
    }
    if (!kodeCocok(kode, tersimpan.kode)) {
      await this.repo.catatPercobaanKode(serahTerimaId);
      throw new BadRequestException({
        code: 'KODE_SALAH',
        message: 'Kode konfirmasi tidak cocok',
        details: { sisaPercobaan: BATAS_PERCOBAAN_KODE - tersimpan.percobaan - 1 },
      });
    }
  }

  private periksaTautan(serahTerimaId: number, penggunaId: number, tautan: string | null | undefined) {
    if (!tautan) {
      throw new BadRequestException({ code: 'TAUTAN_KOSONG', message: 'Tautan tidak disertakan' });
    }
    const hasil = bacaTautan(tautan, this.konf.JWT_RAHASIA);
    if (!hasil.sah || !hasil.isi) {
      throw new ForbiddenException(hasil.alasan ?? 'Tautan tidak sah');
    }
    ...
    if (hasil.isi.serahTerimaId !== serahTerimaId || hasil.isi.penerimaId !== penggunaId) {
      throw new ForbiddenException('Tautan ini bukan untuk serah terima atau penerima ini');
    }
  }
}
