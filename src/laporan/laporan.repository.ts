...
export class LaporanRepository {
   ...
   */
  async efektivitas(tahun: number, unitId?: number) {
   ...
   */
  async pemakaianBulanan(tahun: number, unitId?: number) {
    return this.db.kueri(
      `
   ...
   *      apa adanya.
   */
  async rincianPemakaian(alatId: number) {
    return this.db.kueri(
      `
  ...
  async dashboard(unitId?: number) {
    const syarat = unitId ? 'AND a.unit_id = :unitId' : '';
    const ikatan = unitId ? { unitId } : {};

    const alat = await this.db.kueri(
      `SELECT u.nama AS unit, b.nama AS bidang, rk.nama AS kondisi, COUNT(*) AS jumlah
         FROM alat a
         LEFT JOIN unit u ON u.id = a.unit_id
         LEFT JOIN bidang b ON b.id = a.bidang_id
         LEFT JOIN referensi rk ON rk.id = a.kondisi_id
    ...
    const peminjaman = await this.db.kueri(
      `SELECT pm.status, u.nama AS unit, COUNT(*) AS jumlah
         FROM peminjaman pm
         LEFT JOIN unit u ON u.id = pm.unit_id
