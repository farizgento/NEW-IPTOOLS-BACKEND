...
import type { PilihanKategori, TahapPenilaian } from './penilaian';

@Injectable()
export class PenilaianRepository {
  constructor(private readonly db: OracleService) {}

  async katalog(): Promise<PilihanKategori[]> {
    const baris = await this.db.kueri<Record<string, unknown>>(
      'SELECT id, grup, keterangan, bobot, nilai FROM kategori_kondisi ORDER BY grup, nilai DESC',
    );
    return baris.map((r) => ({
      id: Number(r.ID),
      grup: Number(r.GRUP),
  ...
  async alatPengajuan(peminjamanId: number, tahap: TahapPenilaian) {
  ...
  async simpan(masukan: {
    ...
    oleh: number;
    nama: string;
  }): Promise<{ alat: number; baris: number }> {
      ...
      for (const p of masukan.penilaian) {
        ...
        await koneksi.execute(
          `DELETE FROM penilaian_kondisi
            WHERE peminjaman_alat_id = :paId AND tahap = :tahap AND id_lama IS NULL`,
          { paId: p.peminjamanAlatId, tahap: masukan.tahap },
        );
  ...
  async ringkasan(peminjamanId: number, tahap: TahapPenilaian) {
    return this.db.kueri(
      `
