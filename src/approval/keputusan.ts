...
export type Keputusan = 'MENUNGGU' | 'SETUJU' | 'TOLAK';
...
export const PERAN_TAHAP: Record<TahapApproval, Peran[]> = {
  PENGELOLA: ['PENGELOLA'],
  MANAJER: ['MANAJER'],
  GM: ['GM'],
};
 ...
 */
export function tahapBerjalan(rantai: BarisApproval[]): BarisApproval | undefined {
  return [...rantai]
    .sort((a, b) => a.urutan - b.urutan)
    .find((baris) => baris.keputusan === 'MENUNGGU');
}

export interface AlasanTolak {
  boleh: false;
  alasan: string;
}
export interface Boleh {
  boleh: true;
  baris: BarisApproval;
}
 ...
 */
export function bolehMemutuskan(
  rantai: BarisApproval[],
  peranPengguna: readonly Peran[],
  ...
  const perluPeran = PERAN_TAHAP[berjalan.tahap];
 ...
 */
export function statusSetelahKeputusan(
  rantai: BarisApproval[],
  tahapDiputuskan: TahapApproval,
  keputusan: 'SETUJU' | 'TOLAK',
): string {
  if (keputusan === 'TOLAK') return 'REJECT';
  ...
  if (!berikutnya) return 'APPROVED';
  if (berikutnya.tahap === 'MANAJER') return 'WAPPR MGR';
  if (berikutnya.tahap === 'GM') return 'WAPPR GM';
  return 'APPROVED';
}
 ...
 */
export function periksaAlasan(keputusan: 'SETUJU' | 'TOLAK', alasan?: string | null): string | null {
  if (keputusan !== 'TOLAK') return null;
  if (!alasan?.trim()) return 'Alasan penolakan wajib diisi';
  if (alasan.trim().length < 5) return 'Alasan penolakan terlalu singkat';
  return null;
}

/** Alasan siap pakai, disusun dari alasan yang benar-benar muncul di data lama. */
export const ALASAN_SIAP_PAKAI = [
