...
export const normalNama = (nama: string): string =>
  nama.replace(/\s+/g, ' ').trim().toUpperCase();

export function periksaProyek(isi: IsiProyek): Pelanggaran[] {
  const salah: Pelanggaran[] = [];
  const nama = isi.nama.trim();
  if (!nama) salah.push({ field: 'nama', message: 'Nama pekerjaan wajib diisi' });
  if (nama.length > 255) salah.push({ field: 'nama', message: 'Nama pekerjaan maksimal 255 huruf' });

  for (const [field, nilai] of [
    ['tanggalMulai', isi.tanggalMulai],
    ['tanggalSelesai', isi.tanggalSelesai],
  ] as const) {
    if (nilai && !TANGGAL.test(nilai)) salah.push({ field, message: 'Tanggal tidak sah' });
  }
 ...
 */
export function miripProyek(a: string, b: string): number {
  const potong = (t: string): Set<string> =>
    new Set(
      normalNama(t)
  ...
  const berangka = (s: Set<string>) => [...s].filter((k) => /\d/.test(k));
  const angkaX = berangka(x).sort().join(',');
  const angkaY = berangka(y).sort().join(',');
  if (angkaX !== angkaY) return 0;
