...
import { tandaiTabrakan, type Jadwal, type JadwalBerstatus } from './jadwal';

export interface FilterJadwal {
  dari: string;
  sampai: string;
...
export interface BarisJadwalAlat {
  alatId: number;
  ...
  bidang: string | null;
  bertabrakan: boolean;
  jadwal: (JadwalBerstatus & { pekerjaan: string | null; peminjam: string | null })[];
}

interface BarisMentah extends Record<string, unknown> {
  PA_ID: number;
...
export class JadwalRepository {
   ...
   */
  async perAlat(filter: FilterJadwal, hariIni: string): Promise<BarisJadwalAlat[]> {
    ...
    const rows = await this.db.kueri<BarisMentah>(
      `
      WITH terpilih AS (
    ...
    const peta = new Map<number, BarisJadwalAlat & { mentah: Jadwal[]; ket: Map<number, BarisMentah> }>();
    for (const b of rows) {
    ...
    return [...peta.values()].map((baris) => {
      const jadwal = tandaiTabrakan(baris.mentah, hariIni).map((j) => {
  ...
  > {
    ...
    const ikatan: Record<string, unknown> = { mulai, selesai };
    const penanda = alatIds.map((id, i) => {
      ikatan[`alat${i}`] = id;
      return `:alat${i}`;
    });
    if (kecualiPeminjamanId) ikatan.kecuali = kecualiPeminjamanId;
