...
export type JenjangPengingat = 'H1' | 'H3' | 'H7' | 'JATUH_TEMPO' | 'TERLAMBAT';

export interface AturanJenjang {
  jenjang: JenjangPengingat;
  /** Berapa hari setelah peristiwa acuan pengingat ini jatuh tempo. */
  setelahHari: number;
  perihal: string;
}

/** Pengingat konfirmasi penerimaan, dihitung sejak alat diserahkan. */
export const JENJANG_KONFIRMASI: AturanJenjang[] = [
 ...
 */
export function jenjangJatuhTempo(
  diserahkanPada: Date,
  sekarang: Date,
  sudahDikirim: readonly JenjangPengingat[],
  aturan: readonly AturanJenjang[] = JENJANG_KONFIRMASI,
): AturanJenjang | undefined {
 ...
 */
export function pengingatPengembalian(
  tanggalSelesai: Date,
  sekarang: Date,
  sudahDikirim: readonly JenjangPengingat[],
): AturanJenjang | undefined {
 ...
 * mana pun (PRD F12).
 */
export function susunPesan(masukan: {
  jenjang: JenjangPengingat;
  namaPenerima: string;
  jumlahAlat: number;
  nomorPeminjaman: number;
  umurHari: number;
}): string {
  ...
  if (jenjang === 'TERLAMBAT') {
    return (
      `${namaPenerima}, ${jumlahAlat} alat pada peminjaman #${nomorPeminjaman} ` +
