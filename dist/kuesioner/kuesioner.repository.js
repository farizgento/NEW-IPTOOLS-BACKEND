...
exports.KuesionerRepository = void 0;
const common_1 = require("@nestjs/common");
const oracledb_1 = __importDefault(require("oracledb"));
const oracle_service_1 = require("../basisdata/oracle.service");
const kuesioner_1 = require("./kuesioner");
let KuesionerRepository = class KuesionerRepository {
    db;
    constructor(db) {
        this.db = db;
    }
    async pertanyaanAktif() {
        const baris = await this.db.kueri(`SELECT id, kategori, pertanyaan FROM pertanyaan_kuesioner
    ...
    async ambil(id) {
        const sesi = await this.db.kueriSatu(`SELECT k.id, k.peminjaman_id, k.status, k.nama_perusahaan, k.nama_unit,
        ...
        if (!sesi)
            return undefined;
        const jawaban = await this.db.kueri(`SELECT j.pertanyaan_id, pt.kategori, pt.pertanyaan, j.kepentingan, j.kinerja
         FROM kuesioner_jawaban j
         LEFT JOIN pertanyaan_kuesioner pt ON pt.id = j.pertanyaan_id
        WHERE j.kuesioner_id = :id
        ORDER BY pt.urutan, j.pertanyaan_id`, { id });
        return { sesi, jawaban };
    }
    async buat(masukan) {
        return this.db.transaksi(async (koneksi) => {
            const hasil = await koneksi.execute(`INSERT INTO kuesioner (peminjaman_id, tanggal, nama_perusahaan, nama_unit,
         ...
         RETURNING id INTO :id`, { ...masukan, id: { dir: oracledb_1.default.BIND_OUT, type: oracledb_1.default.NUMBER } });
    ...
    async simpanJawaban(masukan) {
        const bentuk = new Map(masukan.pertanyaan.map((p) => [p.id, (0, kuesioner_1.bentukPertanyaan)(p)]));
        return this.db.transaksi(async (koneksi) => {
            for (const j of masukan.jawaban) {
                const teks = bentuk.get(j.pertanyaanId) === 'TEKS';
                await koneksi.execute(`MERGE INTO kuesioner_jawaban t
           USING (SELECT :kuesionerId AS kuesioner_id, :pertanyaanId AS pertanyaan_id FROM dual) s
            ...
            const hasil = await koneksi.execute(`SELECT pertanyaan_id FROM kuesioner_jawaban WHERE kuesioner_id = :id`, { id: masukan.kuesionerId }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
            const terjawab = (hasil.rows ?? []).map((r) => Number(r.PERTANYAAN_ID));
            const aktif = masukan.pertanyaan.map((p) => p.id);
            const status = (0, kuesioner_1.sesiLengkap)(aktif, terjawab) ? 'SUBMIT' : 'DRAFT';
    ...
    async barisRekap() {
        const baris = await this.db.kueri(`SELECT j.pertanyaan_id, j.kepentingan, j.kinerja
         FROM kuesioner_jawaban j
    ...
    async masukanTeks() {
        return this.db.kueri(`SELECT pt.kategori, j.kepentingan AS isi,
...
exports.KuesionerRepository = KuesionerRepository;
exports.KuesionerRepository = KuesionerRepository = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [oracle_service_1.OracleService])
], KuesionerRepository);
//# sourceMappingURL=kuesioner.repository.js.map