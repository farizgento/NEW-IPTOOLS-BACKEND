...
exports.MasterRepository = exports.TIPE_REFERENSI = void 0;
const common_1 = require("@nestjs/common");
const oracledb_1 = __importDefault(require("oracledb"));
const oracle_service_1 = require("../basisdata/oracle.service");
exports.TIPE_REFERENSI = [
    'JENIS_ALAT',
    'KONDISI_ALAT',
    'LOKASI_ALAT',
    'TIPE_OH',
...
let MasterRepository = class MasterRepository {
    ...
    async tambahReferensi(tipe, kode, nama) {
        await this.db.transaksi(async (koneksi) => {
            await koneksi.execute(`INSERT INTO referensi (tipe, kode, nama, urutan)
         VALUES (:tipe, :kode, :nama, (SELECT NVL(MAX(urutan),0)+1 FROM referensi WHERE tipe = :tipe))`, { tipe, kode, nama });
        });
    }
    async ubahReferensi(id, nama, aktif) {
        await this.db.transaksi(async (koneksi) => {
            await koneksi.execute(`UPDATE referensi SET nama = :nama, aktif = :aktif, dari_migrasi = 0 WHERE id = :id`, { nama, aktif: aktif ? 1 : 0, id });
        });
    }
    /**
     ...
     */
    async gabungReferensi(dariId, keId) {
        return this.db.transaksi(async (koneksi) => {
            let dialihkan = 0;
            for (const kolom of ['jenis_id', 'kondisi_id', 'lokasi_id']) {
                const hasil = await koneksi.execute(`UPDATE alat SET ${kolom} = :keId WHERE ${kolom} = :dariId`, { keId, dariId });
    ...
    async tetapkanUnit(penggunaId, unitId, sumber) {
        await this.db.transaksi(async (koneksi) => {
            await koneksi.execute(`UPDATE pengguna SET unit_id = :unitId, sumber_unit = :sumber WHERE id = :penggunaId`, { unitId, sumber, penggunaId });
        });
    }
    async unit(termasukNonaktif = false) {
        return this.db.kueri(`SELECT u.id, u.nama, u.kode_re, u.kode_maximo, u.deskripsi,
    ...
    async unitSatu(id) {
        return this.db.kueriSatu(`SELECT id, nama, kode_re, kode_maximo, deskripsi,
              TO_CHAR(nonaktif_pada,'YYYY-MM-DD') AS nonaktif_pada
         FROM unit WHERE id = :id`, { id });
    }
    /** Nama unit tidak boleh kembar; pemeriksaannya di sini karena butuh basis data. */
    async unitBernamaSama(nama, kecualiId) {
        const baris = await this.db.kueriSatu(`SELECT 1 AS ada FROM unit
        WHERE UPPER(TRIM(nama)) = :nama ${kecualiId ? 'AND id <> :kecualiId' : ''}
        FETCH FIRST 1 ROWS ONLY`, kecualiId
    ...
    async tambahUnit(isi) {
        return this.db.transaksi(async (koneksi) => {
            const hasil = await koneksi.execute(`INSERT INTO unit (nama, kode_re, kode_maximo, deskripsi, id_lama)
     ...
     */
    async ubahUnit(id, isi) {
        await this.db.transaksi(async (koneksi) => {
            await koneksi.execute(`UPDATE unit
    ...
    async proyek(filter) {
        ...
        if (filter.tanpaTipeOh)
            syarat.push('p.tipe_oh_id IS NULL');
        const where = syarat.length ? `WHERE ${syarat.join(' AND ')}` : '';
        const jumlah = await this.db.kueriSatu(`SELECT COUNT(*) AS total FROM proyek p ${where}`, ikatan);
    ...
    async proyekSatu(id) {
        return this.db.kueriSatu(`SELECT id, TRIM(nama_pekerjaan) AS nama,
         ...
         FROM proyek WHERE id = :id`, { id });
    }
    async proyekBernamaSama(nama, kecualiId) {
        const baris = await this.db.kueriSatu(`SELECT 1 AS ada FROM proyek
    ...
    async tambahProyek(isi) {
        return this.db.transaksi(async (koneksi) => {
            const hasil = await koneksi.execute(`INSERT INTO proyek (nama_pekerjaan, tanggal_mulai, tanggal_selesai, siteco, tipe_oh_id, id_lama)
         VALUES (:nama,
    ...
    async ubahProyek(id, isi) {
        await this.db.transaksi(async (koneksi) => {
            await koneksi.execute(`UPDATE proyek
    ...
    async jumlahPekerjaanLepas() {
        const baris = await this.db.kueriSatu(`SELECT COUNT(*) AS pengajuan, COUNT(DISTINCT UPPER(TRIM(pm.pekerjaan))) AS unik
         FROM peminjaman pm
        WHERE pm.pekerjaan IS NOT NULL
          AND TRIM(pm.pekerjaan) IS NOT NULL
          AND NOT EXISTS (SELECT 1 FROM proyek p
    ...
    async bidang() {
        return this.db.kueri(`SELECT id, nama, dari_migrasi,
...
exports.MasterRepository = MasterRepository = __decorate([
